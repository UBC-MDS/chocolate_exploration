...
Nevertheless, popularity can vary between products. How well-received a new chocolate may be can depend on a variety of factors. The project presented here is interested in predicting the rating for a type of dark chocolate on a scale of 1 to 5, given some of its characteristics. This information can be useful for product analysis, and may be able to aid in predicting the popularity of a new product.
...
The models performed well on the test data sets, with similar results compared to the validation scores, as shown in Table 3. The best-performing model was the SVM RBF model, as it only has an average of 8.6% error for its ratings, while the worst-performing model was the decision tree model, with an average of 11% error. This means, for example, that if the model predicted a rating to be 4, it is possible that the actual rating was around 3.7 or 4.3. Although this is not extremely different, since our rating scale is quite small, we would still prefer to decrease the error if possible.
